...
function SelectLabel({
  ...
  return (
    <SelectPrimitive.Label
      ...
      className={cn("text-muted-foreground flex min-h-9 items-center px-3 py-2 text-xs", className)}
      {...props}
    />
  )
}
...
function SelectItem({
  ...
  return (
    <SelectPrimitive.Item
      ...
      className={cn(
        "focus:bg-accent/18 focus:text-accent-foreground [&_svg:not([class*='text-'])]:text-muted-foreground relative flex min-h-11 w-full cursor-default items-center gap-2 rounded-md py-2.5 pr-8 pl-3 text-sm outline-hidden select-none transition-colors data-[disabled]:pointer-events-none data-[disabled]:opacity-50 [&_svg]:pointer-events-none [&_svg]:shrink-0 [&_svg:not([class*='size-'])]:size-4 *:[span]:last:flex *:[span]:last:items-center *:[span]:last:gap-2",
        className
      )}
      {...props}
    >
